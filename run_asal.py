...
def rollout_boids(theta, steps=400, size=256, vlm=None, capture_every=10, seed=0):
    """Run a Boids simulation and (optionally) collect VLM embeddings.
    Returns: (frames[PIL], embeddings[list[np.ndarray]])
    """
    env = Boids(seed=seed)
    env.reset(theta)
    frames, embs = [], []
    for t in range(steps):
        env.step()
        if (t+1) % max(1, capture_every) == 0 or t == steps-1:
...
def main():
    ...
    if args.mode == 'target':
        def eval_theta(theta):
            _, embs = rollout_boids(theta, steps=args.steps, vlm=vlm, capture_every=cap_every, seed=args.seed)
        ...
        frames, _ = rollout_boids(best, steps=args.steps, vlm=vlm, capture_every=cap_every, seed=args.seed)
        ...
        for i, th in enumerate(pool):
            frames, embs = rollout_boids(th, steps=args.steps, vlm=vlm, capture_every=cap_every, seed=args.seed)
